...
export async function POST(request: NextRequest) {
    try {
        ...
        // Zaten askıda değil mi kontrol et
        if (targetUserData?.status !== 'suspended') {
            return NextResponse.json(
                { error: 'Bu kullanıcı zaten askıda değil.' },
                { status: 400 }
            );
        }

        // Kullanıcıyı askıdan çıkar
        ...
        console.error('Kullanıcı askıdan çıkarma hatası:', error);
        return NextResponse.json(
            { error: 'Kullanıcı askıdan çıkarma işlemi sırasında bir hata oluştu.' },
            { status: 500 }
        );
    }
}
